...
class TestAuthControllers(TestCase):
    ...
    def test_valid_first_login_positive(self) -> None:
        ...
        self.assertTrue(self.auth_controller_obj.valid_first_login("name","pwd","pwd"))
        self.mock_obj_common.change_default_password.assert_called_once()

    def test_valid_first_login_negative(self) -> None:
        self.assertFalse(self.auth_controller_obj.valid_first_login("name","pw1d","pwd"))
    ...
    @mock.patch('src.controllers.auth_controllers.db')
    def test_validate_user_positive(self, mock_db,mock_cls_obj1,mock_cls_obj2):
        mock_db.fetch_data.return_value = [(("EMPdsse","pwd","admin","false"))]
