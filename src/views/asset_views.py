...
class AssetViews:
    ...
    def add_asset(self) -> None:
        ...
        if not data:
            ...
            CommonHelper.display_table(
                data, Header.SCHEMA_MAPPING_CATGEORY_VENDOR_TABLE
            )
            mapping_id = InputValidations.input_mapping_id()
            purchased_date = InputValidations.input_date()
            asset_type = self.input_asset_type()

            if not self.obj_asset_controller.add_asset_to_inventory(
                asset_type, mapping_id, purchased_date
            ):
                print(PromptConfig.NO_DATA_EXISTS)
            else:
